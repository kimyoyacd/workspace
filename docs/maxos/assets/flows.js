...
window.FLOWS = {
  entry: [
    { label: '일이 들어와요', note: '메일·미팅·메신저 어디로 오든 여기가 출발점이에요.' },
    { label: '새 일인지, 하던 일인지 정해요', note: '하던 일이면 진행 중인 일에서 그 일을 열고 돌아와요.' },
    { label: '어떤 일인지 골라요', note: '만들 결과물만 고르면 돼요.' },
    { label: '작업 공간이 열려요', note: '맡은 사람이 끝까지 이어서 해요. Figma 프로젝트 템플릿을 복제해 새 공간을 만들어요.' }
  ],
  end: [
    { label: '사람이 확인해요', en: 'Human Review', note: '방향을 정할 때 한 번, 내보내기 전에 한 번. 통과·보완·다시 중 하나를 그 자리에서 정해요.' },
    { label: '이 일의 기록으로 남겨요', en: 'Project SoT', note: '무엇을 왜 그렇게 정했는지 한곳에 모아요. 다음 사람이 처음부터 다시 묻지 않게요.' },
    { label: '다시 쓰게 모아둬요', en: 'Asset Library', note: '검수를 지난 것만 올라가요. 지금은 Figma 자산 파일에 모아요.' }
  ],
  branches: [
    {
      id: 'proposal', label: '제안서 만들기',
      when: '아직 우리 일로 확정되지 않아서 먼저 설득해야 할 때 골라요. 만드는 갈래가 아니라 앞단이고, 통과하면 아래 세 갈래 중 하나로 넘어가요. 정식 명칭은 제안·RFP예요.',
      subtypes: [{
    ...
    },
    {
      ...
      when: '확정된 일을 실제로 만들 때 골라요. 무엇을 만드는지에 따라 순서가 달라서 아래에서 한 번 더 골라요.',
      subtypes: [
    ...
    },
    {
      ...
      when: 'AI로 만든 이미지가 필요할 때 골라요. 사람이 나오는 이미지가 기본이고, 제품·목업·배경·글자 이미지도 같은 순서로 만들어요. 정식 명칭은 AI 비주얼 에셋이에요.',
    ...
    },
    {
      ...
      when: '영상이 필요할 때 골라요. 길이에 따라 준비물이 달라서 몇 초짜리인지부터 골라요. 사운드 방향과 음원 권리 확인은 두 유형 모두에 들어 있어요.',
      subtypes: [
