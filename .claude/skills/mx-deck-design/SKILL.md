...
| 스킬 | 좌측 구조 | 언제 |
|---|---|---|
| **mx-deck-design** (이 스킬) | 없음(슬라이드 순서대로) | 클라이언트 제안서 · 컨셉덱 (발표용, 위→아래 진행) |
| `mx-research-dashboard-design` | nav-item 12개 탭(클릭 시 뷰 전환) | **마케터 에이전트 산출 표준** — 리서치·전략 보고서 |
| `list-deck-design` | 앵커 목차(클릭 시 스크롤 이동) | 에디토리얼 리포트 (한 페이지 스크롤형) |

**마케터 에이전트가 리서치 결과를 문서로 낼 때는 `mx-research-dashboard-design` 스킬을 쓴다** —
이 스킬(mx-deck-design)이 아니다. 이 스킬은 "발표"용 슬라이드 덱 전용이다.

## 언제 쓰나

- 클라이언트 제안서(리서치 → 전략 → 컨셉 흐름)를 HTML 슬라이드 덱으로
- Figma 덱을 만들기 전 초안을 빠르게 조판해볼 때

쓰지 말 것: 리서치·전략 보고서(그건 `mx-research-dashboard-design`), 에디토리얼 리포트(그건 `list-deck-design`).
