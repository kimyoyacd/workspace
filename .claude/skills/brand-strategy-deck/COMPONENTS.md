...
- 기본은 모노톤: `--bg #f4f3ef` / `--panel #fff` / `--ink #151515` / `--muted #6f6d66` / `--line #e4e1d9` / 탑바·히어로 `--dark #151817`
- 포인트는 4색만, **분류·구분 전용**: `--pt-blue #2e53f9`(T1·Evidence ID) · `--pt-lime #c5ff79`(T2·긍정 상태) · `--pt-pink #ff86f6`(T3·보조 분류) · `--pt-orange #ff6e23`(T4·리스크)
- 본문·제목·장식·버튼·선택 상태에는 포인트 컬러 금지 — 선택/강조는 `--ink` 반전으로.

## 레이아웃 골격 (v2 — Research & Strategy OS)

- `topbar`(다크, sticky) + `sidebar`(뷰 내비, sticky) + `main > .view` SPA 전환
- 뷰 12개: 00 요약 대시보드 · 01 브리프 · 02 팩트북·타깃 · 03 리서치 설계 · 04 경쟁 리서치 ·
  05 포지셔닝·화이트스페이스 · 06 이종업계 · 07 인사이트 · 08 전략 대안 · 09 추천 전략 ·
  10 출처 추적 · 11 실행 로그·한계
- 반응형: 1100px(2단→1단 그리드) / 740px(사이드바→상단 가로 내비, 전 그리드 1단) / print(전 뷰 펼침)
