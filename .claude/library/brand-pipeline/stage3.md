# 3차 · Brand Systemization & Final Slide Deck Compiler

**File role:** Claude Project Knowledge / Skill resource  
**Stage:** 3차 Hyojung guide  
**Primary function:** 브랜드 시스템화 + 최종 슬라이드 전체 생성  
**Important rule:** 최종 슬라이드는 이 단계에서만 생성한다. 1차, 2차가 완료되지 않았으면 슬라이드를 만들지 않는다.

## ★ 가이드 진행 (질문형 · 게이트)

상세 산출 양식은 아래 본문 사용. 슬라이드/이미지 프롬프트는 Gate 5 통과(`slide_generation_ready=true`) 후에만.

### Stage 3 · Brand System & Slide HTML Export — Gate 5
질문: ① 슬라이드 분량(15·30·45장)? ② 이미지 생성 프롬프트 필요? ③ 최종 HTML 용도(발표용·검토용·Figma 이식용)?
산출: Brand System Summary · Value Cards · Keyword Map · Copy Headline System · Symbol Association · Image Prompt Pack · Slide HTML Structure · Final HTML
Gate 5: 모든 슬라이드가 전략 관점 한 문장으로 환원되나 / 각 슬라이드 Key Message 하나만 / 이미지 프롬프트가 브랜드 시스템과 연결되나 / HTML 라이트모드·가독성·위계 충족하나

### Final Review · Image Prompt + Slide HTML Quality Gate
`slide_generation_ready=true`에서만 이미지 프롬프트 + HTML 슬라이드 생성 → `fact-checker`→`cold-critic`→`delivery-gate`→사용자 최종 OK.
HTML 슬라이드 구조: Cover · Executive Summary · Wide Research · Positioning Map · Strategic POV · 3 Core Keywords · Visual Identity DNA · Concept Territories · Behavior System · Human Touch · Brand System · Image Prompt Pack · Slide Export Notes · Final Review.
...
Brand Board Havnn은 최종 브랜드 시스템의 기준판입니다.  
...
```md
1차과 2차의 state를 기반으로 Brand Board Havnn을 작성하세요.
...
```md
Brand Board Havnn을 기반으로 브랜드의 핵심 가치 카드를 3~5개 작성하세요.
...
```md
Brand Board Havnn과 Value Keyword Map을 기반으로 Copy Headline System을 작성하세요.
...
```md
1차의 경쟁 브랜드 30개 중 주요 브랜드를 선별해 Competitor CI/BI Audit을 작성하세요.
...
```md
Brand Board Havnn, Value Cards, Value Metrics, Value Keyword Map, Copy Headline System, Symbol Association Map, Competitor CI/BI Audit을 기반으로 Brand System Summary를 작성하세요.
...
```md
[ ] Brand Board Havnn이 1차, 2차를 모두 반영하는가?
...
[ ] Brand Board Havnn이 전체 시스템을 요약하는가?
