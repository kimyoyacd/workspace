# 2차 · Visual Identity Extraction & Concept / Moodboard Divergence

**File role:** Claude Project Knowledge / Skill resource  
**Stage:** 2차 Hyojung guide  
**Primary function:** 비주얼 아이덴티티 추출 + 컨셉/무드보드 발산  
**Important rule:** 이 단계에서도 최종 슬라이드를 만들지 않는다. 다음 단계로 넘길 `stage2_state`만 만든다.

## ★ 가이드 진행 (질문형 · 게이트)

질문 3개 이하 · 답변 후 산출 · `Assumptions` 분리 · 슬라이드 HTML 생성 금지. 상세 산출 양식은 아래 본문 사용.

### Stage 2 · Visual Identity & Concept System — Gate 3
질문: ① 보존해야 할 자산? ② 과감히 바꿔도 되는 자산? ③ AI 이미지 휴먼터치 레이어 수준(낮음·중간·높음)?
산출: Visual Identity DNA · Concept Territories 6 · Client×Mood Matrix · Behavior System 초안 · Human Touch Layer · Preserve vs Destroy Map
Gate 3: 컨셉들이 충분히 다른가 / 모두 같은 전략 관점으로 회수되나 / 정적 이미지 아닌 행동 시스템으로 확장되나 / 휴먼터치가 장식 아닌 브랜드 이유와 연결되나

### Stage 2.5 · Concept Selection — Gate 4
질문: ① 가장 설득력 있는 컨셉? ② 클라이언트가 받기 어려운 리스크? ③ A/B/Alt 구조 vs 단일 압축?
Gate 4: A/B가 진짜 다른가 / Alt가 안전망인가 / 리스크 정렬 통과했나
자체검토 4렌즈 + Human Touch + Preserve vs Destroy 적용.
...
```md
1차과 2차의 state를 기반으로 3차 단계로 넘어갑니다.
이제 브랜드 시스템화를 진행합니다.
아직 최종 슬라이드 전체를 만들기 전에, 먼저 브랜드 시스템 산출물을 완성하세요.

해야 할 일:
1. Brand Board Havnn 구성
2. Value Cards 생성
3. Value Metrics 생성
