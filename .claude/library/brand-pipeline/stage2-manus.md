...
Manus 2는 반드시 Manus 1의 state를 입력으로 받습니다.
...
Manus 1 state가 없거나 부족하면, 슬라이드나 컨셉을 만들지 말고 누락된 항목을 먼저 요청합니다.

---

## 2. Core Output

Manus 2의 최종 산출물은 아래 8개입니다.
...
Manus 2는 Manus 1의 포지셔닝 전략을 감각으로 바꿉니다.
...
하지만 모두 Manus 1에서 도출한 포지셔닝 전략으로 회수되어야 합니다.
...
```md
Manus 1의 포지셔닝 전략과 선호도 분석을 기반으로 Visual Identity DNA를 추출하세요.
...
```md
Manus 1의 경쟁 브랜드 리서치와 클러스터링을 기반으로 경쟁사들이 반복적으로 쓰는 시각 코드를 정리하세요.
...
```md
아래 6개 Concept Territory를 Manus 1의 포지셔닝 전략과 Visual Identity DNA에 맞춰 비교하세요.
...
# 10. Manus 2 State Schema

Manus 2를 마칠 때 반드시 아래 형태로 정리합니다.
...
Manus 2 종료 전 반드시 확인합니다.

```md
[ ] Visual Identity DNA가 Manus 1의 포지셔닝 전략과 연결되는가?
...
Manus 2가 완료되면 아래 프롬프트로 Runable 3를 실행합니다.

```md
Manus 1과 Manus 2의 state를 기반으로 Runable 3 단계로 넘어갑니다.
이제 브랜드 시스템화를 진행합니다.
