...
## 💡 아이디어 인박스 운영
- 사용자가 아이디어를 던지면(비즈니스·디자인·서비스 등) 노션 **아이디어 인박스 DB**에 즉시 저장한다 — 주소는 `.claude/library/data-sources.md` 참조.
- 저장 시 자동 태깅: 유형·출처·관련 클라·한 줄 요약. 상태는 `씨앗`으로 시작.
- "확장해줘" 요청 시: brainstormer(발산) + critic(냉정 평가) 병렬 실행 → 종합을 해당 아이디어 페이지 본문에 누적하고 상태를 `확장완료`로, `다음 고민 포인트`를 채운다.
- 규모가 큰 아이디어는 `.claude/projects/`에 프로젝트 노트도 병행 생성한다.
- 노션 read-only 규칙의 예외: 이 DB만 쓰기 허용.
