...
- 응답은 간결하고 실무 중심
- 한국어 소통 (별도 요청 시 영어)
- **리서치·글쓰기 결과물은 항상 `dumbify` + `anti-ai-writing` 스킬을 거친다.** 리서치 리포트·제안서·덱 본문·캡션·메일 등 사람이 읽는 텍스트를 산출하기 전, 두 스킬을 호출해 문장을 정리한다. (조각 표현·전문 용어 남발·AI 티 제거, 쉬운 문장, 모든 수치에 출처)
- **리서치는 경쟁사 → XY 포지셔닝 → 화이트스페이스 순서로 시작한다.** 케이스·성과 조사에 앞서 경쟁 지형과 우리가 설 자리를 먼저 깐다.
