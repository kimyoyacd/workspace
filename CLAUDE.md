...
- **새 에이전트는 반드시 아래 6개 파이프라인 그룹(착수판단/리서치/아이데이션/무드보드·비주얼/검수게이트/운영조직) 중 하나에 배정**해서 등록한다 — "어느 직무팀 소속인가"가 아니라 "파이프라인 어느 단계에서 호출되는가"로 분류한다. 애매하면 그 에이전트가 다른 에이전트의 산출물을 입력으로 받는지(뒷단) 아니면 나가는지(앞단)로 판단.
...
## 현재 등록된 에이전트 (24종 · 파이프라인 흐름 기준 6그룹, 메타 그룹 별도 1종 포함 총 25종)
...
**④ 무드보드·비주얼 제작 그룹** — 관점·리서치를 실제 이미지/시스템으로 만드는 단계. 내부 조직도는 `MOODBOARD_MASTER_FRAMEWORK.md` §10에 별도로 고정되어 있음(무드보드만 단독 의뢰돼도 이 조직도를 그대로 따름). 순서: `moodboard-builder`(총괄 조립) ⇄ `visual-generator`(Higgsfield 이미지, 6패널 그리드 규칙) ⇄ `figma-bridge`(Figma 반영) / `storyboard-maker`(영상 콘티).
...
**⑥ 운영·조직 그룹** — 프로젝트 밖의 회사 운영(매출·인력·계약).
