...
- 신규 브랜드 기획/디자인 제안 요청이 들어오면(브랜드 리서치·컨셉 발산·비주얼 아이덴티티 등 어느 단계든) **반드시 `.claude/library/prompts/brand-design-proposal.md`(마스터 오케스트레이션 가이드)를 자동으로 먼저 확인**하고 그 순서를 따른다 — 사용자가 매번 다시 요청하지 않아도 브랜드 기획/디자인 리서치 작업이면 항상 이 가이드부터 부른다.
