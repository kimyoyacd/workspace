...
| 템플릿 | 파일 | 언제 |
|---|---|---|
| **슬라이드 덱** | `template.html` | 클라이언트 제안서 · 컨셉덱 (발표용 슬라이드 흐름) |
| **리서치 대시보드** | `research-dashboard.html` | **마케터 에이전트 산출 표준** — 리서치·전략 보고서 (사이드바 + 12뷰 탐색형) |

**마케터 에이전트가 리서치 결과를 문서로 낼 때는 반드시 `research-dashboard.html` 구조를 따른다.**
슬라이드 덱은 "발표"용, 리서치 대시보드는 "검토·추적"용이다.

## 언제 쓰나

- 클라이언트 제안서(리서치 → 전략 → 컨셉 흐름)를 HTML 덱으로 → `template.html`
- 마케터 리서치·브랜드 전략 보고서 → `research-dashboard.html`
- Figma 덱을 만들기 전 초안을 빠르게 조판해볼 때

쓰지 말 것: 에디토리얼 리포트(그건 `list-deck-design`).

## 리서치 대시보드 규격 (`research-dashboard.html` 실측)

마케터 4챕터 산출이 이 12뷰에 1:1로 담긴다:

| 뷰 | 내용 | 마케터 챕터 |
|---|---|---|
| 00 요약 대시보드 | 결론 먼저 + 진행 파이프라인 + 핵심 지표 | 종합 |
| 01 브리프·문제 정의 | 요청 → 진짜 결정 문제 재정의 | 0 · CH3 |
| 02 브랜드 팩트북 | 브랜드 사전 이해 | CH1 |
| 03 경쟁 리서치 (30) | 경쟁 브랜드 표 + 출처 | CH1 |
| 04 포지셔닝 맵 | 축·좌표 근거 | CH2 |
| 05 화이트스페이스 | 빈자리 기회/함정 판정 | CH2 |
| 06 이종업계 (30) | 타 업계 차용 사례 | CH1 |
| 07 핵심 인사이트 | 패턴·긴장·So What | CH2→CH3 |
| 08 브랜드 밸류 키워드 | 가치 키워드 도출 | CH3 |
| 09 전략 대안 (3) | 방향 후보 비교·점수 | CH4 |
| 10 추천 전략 | 추천안 + 실행 원칙 + 하지 않을 것 | CH4 |
| 11 출처·근거 추적 | 전체 출처 티어 분류 | 전 챕터 |

**리서치 대시보드 토큰** (슬라이드 덱과 다름 — 모노톤 + 포인트 4색 고정):
```css
--bg:#f2f1ee; --panel:#fff; --ink:#161616; --muted:#77756e; --line:#e3e1da;
/* 포인트 4색 — 분류·강조가 꼭 필요한 곳에만. 새 색 생성 금지 */
--pink:#ff86f6; --blue:#2e53f9; --lime:#c5ff79; --orange:#ff6e23;
/* 의미 배정: blue=T1·핵심정보 / lime=T2·긍정·기회 / pink=T3·보류·중간위험 / orange=T4·위험·회피 */
```

사용법: `research-dashboard.html`을 복사해 텍스트·데이터만 치환. 뷰 개수는 프로젝트에 맞게
가감 가능하되(예: RFP 없으면 브리프 뷰 축소), **00 요약·11 출처 추적은 절대 빼지 않는다.**
