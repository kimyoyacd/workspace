...
차트 바, 도넛, 스택바 등 데이터 시각화 요소는 그레이스케일 톤으로 구분:
...
- **우측**: 2열 그리드, 섹션별 그룹핑
  - 섹션 헤더: 번호 + 대문자 타이틀 + 하단 라인 (`rgba(255,255,255,0.15)`)
  - 항목: 번호(11px, 40% opacity) + 라벨(14px, 85% opacity)
- **Footer**: 좌측 문서명, 우측 연도
...
### Chart Bar (Slide 15 월별 차트)

```
/* 라이트→다크 그레이 그래디언트 (월별 진행) */
background: linear-gradient(180deg, #AAAAAA 0%, #D0D0D0 100%);  /* Jan-Feb */
background: linear-gradient(180deg, #999999 0%, #C8C8C8 100%);  /* Mar-Apr */
background: linear-gradient(180deg, #777777 0%, #B8B8B8 100%);  /* May-Jun */
background: linear-gradient(180deg, #666666 0%, #A8A8A8 100%);  /* Jul-Aug */
...
### Process Steps (Slide 18)

```
step-dot: 그레이스케일 (#CCCCCC → #444444, 점점 진해짐)
timeline bar: 그레이 하단 보더 (2px solid #CCC~#444)
timeline bg: rgba(0,0,0, 0.03~0.07) 단계적 진행
```

### Positioning Map (Slide 19)
...
├── <style>                    ← CSS 변수, 컴포넌트, 슬라이드 타입별 스타일
├── <div class="deck">         ← 슬라이드 컨테이너
│   ├── Slide 01: Cover        ← 다크, grid 3-row
│   ├── Slide 02: TOC          ← 다크, grid 3-row
│   ├── Slide 03~06: Content   ← 라이트
│   ├── Slide 07: Divider §02  ← 다크
│   ├── Slide 08~10: Content   ← 라이트
│   ├── Slide 11: Divider §03  ← 다크
│   ├── Slide 12~13: Content   ← 라이트
│   ├── Slide 14: Divider §04  ← 다크
│   ├── Slide 15~16: Content   ← 라이트
│   ├── Slide 17: Divider §05  ← 다크
│   ├── Slide 18~19: Content   ← 라이트
│   ├── Slide 20: Divider §06  ← 다크
│   ├── Slide 21: Content      ← 라이트
